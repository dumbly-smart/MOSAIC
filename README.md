...
The local implementation contains a Python verification engine, a nine-scenario terminal demo, and a layered synthetic document benchmark. Docling/RapidOCR extraction is validated on the complete synthetic 500-page PDF. Ollama BGE-M3 embedded all 1,979 page-bounded chunks and achieved 5/5 ground-truth evidence recall at top-5; a persisted local test index and answer-key-isolated evaluator reproduce the result. PostgreSQL/pgvector integration remains with the backend team. Portals, FastAPI, document upload, and the dashboard are not connected. The original demo and text baseline need no third-party packages (Python 3.11+).
...
See [layer-by-layer setup and testing](docs/verification-benchmark.md) for PDF extraction, database setup, and real model commands. The default `qwen3-vl:4b-instruct` model has now completed all five criteria after BGE-M3 retrieval, using PDF images only for scanned candidate pages. Every status, value, exact quote, page, and point allocation matched the isolated synthetic answer key, producing the expected 35/100 advisory score. All work remains local; nothing has been pushed.

To test a separate tender PDF and bidder PDF without predefined `criteria.json`, use the
[custom PDF workflow](docs/custom-pdf-workflow.md). Qwen3-VL extracts quote-grounded numeric
criteria from the tender, while Docling/RapidOCR, BGE-M3, Qwen3-VL and the deterministic scorer
process the bidder evidence. The included synthetic external test uses an entirely image-only
12-page bidder PDF and passes both rule-extraction and final-report answer-key audits.
